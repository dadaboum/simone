class MessagePreOpJob < ApplicationJob
  ...
  def perform
    ...
    req_pre_op2 = "SELECT last_name, first_name, phone_number, description, id_s FROM (" + req_pre_op1 + ")T1 LEFT JOIN Events ev ON id_s=ev.surgery_id and description='message pré-opératoire envoyé'"
