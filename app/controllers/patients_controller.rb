class PatientsController < ApplicationController
  ...
  def sms_content_patient_and_event(surgery)
    ...
    if Date.today < surgery.date
      message = "Bonjour, à la veille de votre opération, une rapide évaluation de votre état de santé est nécessaire, merci de répondre aux questions suivantes : https://davidbenamran.typeform.com/to/#{typeform_pre}?surgery_id=#{surgery.id}"
      event = Event.new(description: "relance message pré-opératoire", flag: "green")
    else
      message = "Bonjour, au lendemain de votre opération, une rapide évaluation de votre état de santé est nécessaire, merci de répondre aux questions suivantes : https://davidbenamran.typeform.com/to/#{typeform_post}?surgery_id=#{surgery.id}"
