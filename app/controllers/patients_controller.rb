class PatientsController < ApplicationController
  def relance_sms
    ...
    if Date.today < surgery.date
      #https://davidbenamran.typeform.com/to/Dcyfaj?surgery_id=19
      message = "Bonjour, #{patient.last_name} #{patient.first_name}, lien du questionnaire pré-opératoire : https://davidbenamran.typeform.com/to/Dcyfaj?surgery_id=#{params[surgery_id]}"
      event = Event.new(description: "relance message pré-opératoire", flag: "green")
    else
      message = "Bonjour, #{patient.last_name} #{patient.first_name}, lien du questionnaire post-opératoire : https://davidbenamran.typeform.com/to/yccqO5?surgery_id=#{params[surgery_id]}"
